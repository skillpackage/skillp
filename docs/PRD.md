# skilladd Package Manager PRD

> 将 skilladd 从 "add-only" 工具演进为完整的 SKILL 包管理器，类似 Go modules / npm。

## 1. Problem Statement

当前 `skilladd` 只支持 `add` 命令 — 从 git 仓库克隆、发现 SKILL.md、安装到 agent 目录。缺少：
...
```
                    ...
                    └──────┬──────┘
                           │
                    ┌──────▼──────┐
  skilladd add ───► │   skilladd  │ ◄─── skilladd update
  skilladd install  │   核心逻辑   │      skilladd remove
                    └──────┬──────┘
                           │
              ┌────────────┼────────────┐
              ▼            ▼            ▼
...
### 5.1 `skilladd add <source>` (增强现有)

从远程源添加 skill 到项目。

```bash
# SSH 格式
bunx skilladd add git@git.woa.com:chong/chong-skills.git --skill iwiki-ultra

# HTTPS 格式
bunx skilladd add https://git.woa.com/chong/chong-skills.git --skill iwiki-ultra

# 预览链接（无 .git 后缀，从浏览器地址栏复制）
bunx skilladd add https://git.woa.com/chong/chong-skills --skill iwiki-ultra

# GitHub 仓库
bunx skilladd add git@github.com:owner/repo.git --skill my-skill

# 非交互模式（AI Agent 使用）
bunx skilladd add git@git.woa.com:chong/chong-skills.git --skill iwiki-ultra -y
```
...
### 5.2 `skilladd install` (新增)

根据 `skills.json` 恢复 symlinks。不需要网络。

```bash
bunx skilladd install
```
...
4. 如果 `.agents/skills/` 中缺失某个 skill（文件被误删），提示用户运行 `skilladd update`
...
### 5.3 `skilladd update <name>` (新增)

更新已安装的 skill 到最新版本。

```bash
# 更新单个 skill
bunx skilladd update iwiki-ultra

# 更新全部
bunx skilladd update --all

# 非交互模式
bunx skilladd update --all -y
```
...
### 5.4 `skilladd remove <name>` (新增)

移除已安装的 skill。

```bash
bunx skilladd remove iwiki-ultra
```
...
现有用户已经用 `skilladd add` 安装了 skills，但没有 `skills.json`。
...
- 现有 `skilladd add` 命令行为不变，只是额外写入 `skills.json`
...
- [ ] `skilladd outdated`：检查哪些 skills 有可用更新
- [ ] `skilladd doctor`：诊断安装状态（symlinks 是否完整等）
- [ ] Telemetry 安装统计
