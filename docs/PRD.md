...
```jsonc
{
  ...
  "skills": {
    "iwiki-ultra": {
      "source": "git@github.com:chong/chong-skills.git",
...
| `sourceType` | 源平台类型：`"github"` / `"github"` / `"gitlab"` / `"other"` |
| `path` | skill 在源仓库中的路径 |
...
```bash
# SSH 格式
npx --yes skillp add git@github.com:chong/chong-skills.git --skill iwiki-ultra

# HTTPS 格式
npx --yes skillp add https://github.com/chong/chong-skills.git --skill iwiki-ultra

# 预览链接（无 .git 后缀，从浏览器地址栏复制）
npx --yes skillp add https://github.com/chong/chong-skills --skill iwiki-ultra

# GitHub 仓库
npx --yes skillp add git@github.com:owner/repo.git --skill my-skill

# 非交互模式（AI Agent 使用）
npx --yes skillp add git@github.com:chong/chong-skills.git --skill iwiki-ultra -y
```
...
```bash
# 更新单个 skill
npx --yes skillp update iwiki-ultra

# 更新全部
npx --yes skillp update --all

# 非交互模式
npx --yes skillp update --all -y
```
...
```bash
npx --yes skillp remove iwiki-ultra
```
...
# 方式 2: 通过 wget 下载安装脚本（自动确保 Node.js 22+）
wget -qO- https://skills.example.com/install.sh | bash -s -- add git@github.com:chong/chong-skills.git --skill iwiki-ultra
```
...
```bash
#!/bin/bash
# 1. 确保 Node.js 22+ 可用（通过 nvm 安装）
MIN_NODE_VERSION=22
if ! command -v node &> /dev/null || [ "$(node --version | sed 's/^v//' | cut -d. -f1)" -lt "$MIN_NODE_VERSION" ]; then
    echo "Installing Node.js ${MIN_NODE_VERSION} via nvm..."
...
https://github.com/chong/chong-skills            → github.com   chong    chong-skills
git@github.com:chong/chong-skills.git            → github.com   chong    chong-skills
https://github.com/chong/chong-skills.git        → github.com   chong    chong-skills
```
...
- [ ] install.sh 一键安装脚本（自动确保 Node.js 22+）
