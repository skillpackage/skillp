...
```bash
npx --yes skillp add <source>       # 从 git 仓库添加 skill
npx --yes skillp install            # 恢复 symlinks（clone 项目后执行，无需网络）
npx --yes skillp update <name>      # 更新 skill 到最新版本
npx --yes skillp update --all       # 更新全部 skills
npx --yes skillp remove <name>      # 移除已安装的 skill
```
...
```bash
npx --yes skillp add anthropics/skills              # GitHub 简写
npx --yes skillp add https://github.com/org/repo    # HTTPS URL
npx --yes skillp add git@github.com:org/repo.git    # SSH URL
npx --yes skillp add ./my-local-skills              # 本地路径
```
...
```bash
npx --yes skillp update iwiki-ultra     # 更新指定 skill
npx --yes skillp update --all           # 更新全部
npx --yes skillp update --all -y        # 非交互模式（AI Agent 使用）
```
