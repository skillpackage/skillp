...
import { cloneRepo, cloneWithFallback, cleanupTempDir, getCommitInfo, GitCloneError } from './git.ts';
...
import { addSkillToLocalLock } from './local-lock.ts';
import { normalizeUrl, toSshUrl, detectSourceType } from './url.ts';
import { addSkillToSkillsJson, ensureSkillsJson, readSkillsJson, AGENT_DIR_MAP } from './skills-json.ts';
...
export async function promptForAgents(
  ...
  const defaultAgents: AgentType[] = ['claude-code', 'codebuddy', 'codex'];
...
export async function runAdd(args: string[], options: AddOptions = {}): Promise<void> {
  ...
  if (!source) {
    ...
    console.log(`    ${pc.cyan('npx addskills add')} ${pc.yellow('<source>')} ${pc.dim('[options]')}`);
    console.log();
    console.log(pc.dim('  Example:'));
    console.log(`    ${pc.cyan('npx addskills add')} ${pc.yellow('vercel-labs/agent-skills')}`);
    console.log();
    process.exit(1);
  }
  ...
  try {
    ...
    if (parsed.type === 'local') {
      ...
      // Clone repository for remote sources — try SSH first, fallback to HTTPS
      spinner.start('Cloning repository...');
      const normalized = normalizeUrl(parsed.url);
      if (normalized) {
        const result = await cloneWithFallback(normalized, parsed.ref);
    ...
    // Add to local lock file + skills.json for project-scoped installs
    if (successful.length > 0 && !installGlobally) {
      // Extract commit info from the cloned repo
      let commitInfo = { commit: '', commitAt: '' };
      if (tempDir) {
        try {
          commitInfo = await getCommitInfo(tempDir);
        } catch {
          // Non-fatal: commit info is best-effort
        }
      }

      const normalized = normalizeUrl(parsed.url);
      const sshSource = normalized ? toSshUrl(normalized) : (lockSource || parsed.url);
      const sourceType = normalized ? detectSourceType(normalized.host) : 'other';

      const successfulSkillNames = new Set(successful.map((r) => r.skill));
      for (const skill of selectedSkills) {
        const skillDisplayName = getSkillDisplayName(skill);
        if (successfulSkillNames.has(skillDisplayName)) {
          // Compute the skill path relative to the repo root
          let skillPathInRepo: string | undefined;
          const skillPathValue = skillFiles[skill.name];
          if (skillPathValue) {
            // Remove the trailing /SKILL.md to get directory path
            skillPathInRepo = skillPathValue.replace(/\/SKILL\.md$/, '');
          }

          // Read version from SKILL.md frontmatter
          const resolvedVersion = (skill.metadata?.version as string) || 'latest';
          ...
          try {
            ...
            // Don't fail installation if skills.json update fails
          }
        }
      }

      // Create symlinks for agents declared in skills.json
      try {
